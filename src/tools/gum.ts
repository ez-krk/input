...
export const getPostsWithReaction = async (sdk: any, authority: PublicKey) => {
  ...
  const postsWithReactionPromises = posts.map(async (post: any) => {
    const reactionData = await sdk.reaction.getReactionsByPost(
      new PublicKey(post.address)
    );
    const reactions = reactionData.map(
      (reaction: any) => reaction.reaction_type
    );
...
export const getAllPost = async (sdk: SDK, owner: PublicKey) => {
  ...
  const posts = await Promise.all(
    filteredPosts.map(async (element) => {
      ...
      const txUrl =
        cluster === "devnet"
          ? `https://solana.fm/tx/${txSignature[0].signature}?cluster=devnet-solana`
          : `https://solana.fm/tx/${txSignature[0].signature}?cluster=mainnet-solanafmbeta`;
