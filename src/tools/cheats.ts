...
const POSTS_PATH = join(process.cwd(), "_cheats");

// get the file paths of all available list of posts
function getCheatsFilePaths(): string[] {
  return (
...
export function getCheat(slug: string): Post {
...
// load the post items
export function getCheatItems(filePath: string, fields: string[] = []): Items {
  // create a slug from the mdx file location
  const slug = filePath.replace(/\.mdx?$/, "");
  // get the front matter data and content
  const { data, content } = getCheat(slug);
...
// getting all posts
export function getAllCheats(fields: string[]): Items[] {
  // add paths for getting all posts
  const filePaths = getCheatsFilePaths();
  // get the posts from the filepaths with the needed fields sorted by date
  const posts = filePaths
    .map((filePath) => getCheatItems(filePath, fields))
